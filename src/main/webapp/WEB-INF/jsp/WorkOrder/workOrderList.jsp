...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 工单管理 <span class="c-gray en">&gt;</span> 工单中心 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="text-c">
        <form  method="post" action="findWorkOrderList">
        	<select name="isSolve" class="input-text" style="width:150px;">
                ...
                <c:if test="${isSolve==0}">
                 ...
                 <c:if test="${isSolve!=0&&isSolve!=1}">
                	<option value="1">已解决</option>
                	<option value="0">未解决</option>
                </c:if>
            </select>
            <input type="text" class="input-text" style="width:250px" placeholder="工单标题" name="like" value="${like}">
            <button type="submit" class="btn btn-success" name=""><i class="Hui-iconfont">&#xe665;</i> 搜工单标题</button>
        </form>
    </div>
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="r">共有数据：<strong>${olist.total }</strong> 条</span>
    </div>
    <table class="table table-border table-bordered table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="8">工单列表</th>
        </tr>   
        <tr class="text-c">
        ...
        <tbody>
        <c:forEach items="${olist.dataList}" var="app" varStatus="status">
            <tr class="text-c">
                <td><input type="checkbox" value="${app.id}" name="batch"></td>
                <td>${status.index+1}</td>
                <td>${app.theme }</td>
                <td>${app.eMail}</td>
                <td><fmt:formatDate value="${app.startTime }" pattern="yyyy-MM-dd"/>
                    
                </td>
                <td>
                <c:if test="${app.isSolve==1}">
                	已解决
                </c:if>
                <c:if test="${app.isSolve!=1}">
                	未解决
                ...
                <td>${app.content }</td>
                <td>
 					<a title="详情" href="javascript:;" onclick="classify_edit('公司详情','workOrderEdit','${app.id}','800','600')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe665;</i></a>
 					<a title="下载" href="javascript:;" onclick="window.location.href ='downloadOrderAttachment?id=${app.id }'" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe640;</i></a>
...
<script type="text/javascript">
    ...
    $(function() {
		/* 加载分页  */
		//当前页
		var pageNumber = parseInt('${olist.pageNo}');
		//页面大小
		var pageSize = parseInt('${olist.pageSize}');
		//总页数
		var pages = parseInt('${olist.pages}');
		//总条数
		var pageTotal = parseInt('${olist.total}');
		//页面地址
		var like='${like}';
		var isSolve='${isSolve}';
		var pageStart = "href='findWorkOrderList?like="+like+"&isSolve="+isSolve+"&pageNumber=";
		var pageEnd = "'";
